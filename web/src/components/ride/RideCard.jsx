...
import { getHHmmKST } from '../../utils/formatters';
...
export default function RideCard({ party, onClick }) {
  ...
  const time = getHHmmKST(departureTime);
  const routeFrom =
    pickupName || locationLabel(departure);
  const routeTo =
    destinationName || locationLabel(destination);
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5 text-sm font-bold text-gray-800">
          ...
          <svg className="w-3.5 h-3.5 text-gray-300 shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
