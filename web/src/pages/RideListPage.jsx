...
import { getNextDays, getDateLabel, formatDate, getHHmmKST, toDateKeyKST } from '../utils/formatters';
...
export default function RideListPage() {
  ...
  const loadParties = useCallback(async () => {
    ...
    try {
      const params = {
        direction,
        selectedDate,
        selectedSlot: selectedSlot ?? '12:00',
      };
  ...
  }, [direction, selectedLocation, selectedDate, selectedSlot]);
  ...
  const filteredByBase = useMemo(() => {
    return parties.filter((p) => {
      ...
      const matchDate = toDateKeyKST(p.departureTime) === selectedDate;
  ...
  const periodCounts = useMemo(() => {
    ...
    filteredByBase.forEach((party) => {
      const hour = Number(getHHmmKST(party.departureTime).split(':')[0]);
  ...
  const slotCountMap = useMemo(() => {
    const map = {};
    filteredByBase.forEach((party) => {
      const hhmm = getHHmmKST(party.departureTime);
  ...
  const filtered = useMemo(() => {
    if (selectedSlot) {
      return filteredByBase.filter((p) => getHHmmKST(p.departureTime) === selectedSlot);
    }
    const period = PERIODS.find((p) => p.key === selectedPeriod) || PERIODS[1];
    return filteredByBase.filter((p) => {
      const hour = Number(getHHmmKST(p.departureTime).split(':')[0]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-100/90 via-white to-sky-50/30">
      ...
      {selectedPartyId && (
        <PartyDetailSheet
          ...
          onJoin={handleJoin}
          onLeave={handleLeave}
          onRefresh={() => selectedPartyId && loadPartyDetail(selectedPartyId)}
        />
      )}
    </div>
  );
...
function PartyDetailSheet({ party, loading, actionLoading, onClose, onJoin, onLeave, onRefresh }) {
      ...
      >
        ...
        {loading || !party ? (
          ...
          <div className="space-y-4">
            ...
            {(party.isMember || party.isHost || party.canSetTaxiFare) && (
              <TaxiFareSplit
                partyId={party.id}
                totalTaxiFare={party.totalTaxiFare}
                currentMembers={party.currentCount}
                perPersonFare={party.perPersonFare}
                taxiFareRemainder={party.taxiFareRemainder}
                canSetTaxiFare={party.canSetTaxiFare}
                onUpdated={onRefresh}
              />
            )}
