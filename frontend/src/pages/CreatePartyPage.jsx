...
import { LOCATIONS, GENDER_OPTIONS } from '../utils/constants';
import { getNextDays, getDateLabel, formatDate } from '../utils/formatters';

const initialForm = {
  departure: 'CAMPUS',
  destination: 'WONJU_STATION',
  departureTime: '',
  maxCount: 4,
  genderOption: 'ANY',
};
...
export default function CreatePartyPage() {
  ...
  const locationOptions = useMemo(() => LOCATIONS.filter((l) => l.key !== 'ALL'), []);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const normalizedDepartureTime = normalizeToTenMinute(form.departureTime);
      await createParty({
        ...form,
